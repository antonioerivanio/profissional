...
public class SRHUtils {
	...
	/**
	 * Método responsável por adicionar ou remover número de dias de uma data Obs.:
	 * sinal negativo no parâmetro qtdDias irá fazer com que seja removido o número
	 * de dias informado, caso contrário, será adicionado a mesma, gerando uma nova
	 * data.
	 * 
	...
	/**
	 * Método responsável por retornar a data a partir da qual não se deduz faltas
	 * do tempo de serviço
	 * 
	 * @return data inicio
	 */
	...
	/**
	 * Método com cálculo específico para Averbação que compara duas datas e retorna
	 * o numero de dias de diferença entre elas
	 * 
	 * @param dataLow
	 * @param dataHigh
	 * 
	...
	/**
	 * Método que compara duas datas e retorna o número de anos, meses e dias entre
	 * elas. O cálculo feito no método produz um resultado próximo ao demonstrativo
	 * da simulação do cálculo do tempo de contribuição do INSS
	 * 
	 * @param dataInicio
	 * @param dataFim
	 * 
	...
	public static int[] contagemDeTempoDeServico(Date dataInicio, Date dataFim) {
		...
		// Caso Especial: Data final corresponde ao último dia de fevereiro e data
		// inicial começa no dia 1º
	...
	/**
	 * Formata uma data de acordo com o parâmetro passado Ex. parâmetro formato:
	...
	public static boolean isCNPJ(String CNPJ) {
		...
		// considera-se erro CNPJ's formados por uma sequencia de numeros iguais, vazio
		// ou nulo
	 ...
	 * na qual esta deve ser informada no seguinte padrão: NNNN-D, onde N é um valor
	 * numerico e D um valor numerico que representa o dígito verificador gerado a
	 * partir dos outros 4 números informados antes do hífen (-).
	 * 
	 * @param agencia
	 * @return Boolean
	 */
	public static Boolean validarDVAgenciaBradesco(String agencia) {
		...
		int soma = Integer.parseInt(agencia.substring(0, 1)) * 5 + Integer.parseInt(agencia.substring(1, 2)) * 4
				+ Integer.parseInt(agencia.substring(2, 3)) * 3 + Integer.parseInt(agencia.substring(3, 4)) * 2;
	...
	/**
	 * Método que verifica se o Dígito Verificador de uma Conta Corrente Bradesco é
	 * válida, na qual esta deve ser informada no seguinte padrão: NNNNNNN-D, onde N
	 * é um valor numerico e D um valor numerico que representa o dígito verificador
	 * gerado a partir dos outros sete números informados antes do hífen (-).
	 * 
	 * @param conta
	 * @return Boolean
	 */
	public static Boolean validarDVContaCorrenteBradesco(String conta) {
		...
		soma = Integer.parseInt(conta.substring(1, 2)) * 7 + Integer.parseInt(conta.substring(2, 3)) * 6
				+ Integer.parseInt(conta.substring(3, 4)) * 5 + Integer.parseInt(conta.substring(4, 5)) * 4
				+ Integer.parseInt(conta.substring(5, 6)) * 3 + Integer.parseInt(conta.substring(6, 7)) * 2;
	...
	public static boolean validarMatricula(String matricula) {
		int dv = 0;
		dv = ((Integer.valueOf(matricula.substring(0, 1)) * 8) + (Integer.valueOf(matricula.substring(1, 2)) * 9)
				+ (Integer.valueOf(matricula.substring(2, 3)) * 8) + (Integer.valueOf(matricula.substring(3, 4)) * 7))
				% 10;
	...
	public static BigDecimal valorMonetarioStringParaBigDecimal(String valorMonetarioString) {
		...
		for (int i = 0; i < valorMonetarioString.length(); i++) {
			if (valorMonetarioString.charAt(i) != ',' && valorMonetarioString.charAt(i) != '.'
					&& valorMonetarioString.charAt(i) != 'R' && valorMonetarioString.charAt(i) != '$')
	...
	public static BigDecimal valorMonetarioStringParaBigDecimal2(String valorMonetarioString) {
		String valor = "";
		for (int i = 0; i < valorMonetarioString.length(); i++) {
			if (valorMonetarioString.charAt(i) != '.' && valorMonetarioString.charAt(i) != 'R'
					&& valorMonetarioString.charAt(i) != '$')
	...
	public static Boolean validarPisPasep(String pisPasep) {
		...
		// verifica se a quantidade de caracteres que compoe o PIS/PASEP foram
		// informados.
	...
	public static boolean validarProcesso(String processo) {
		...
		String nDigVerific = processo.substring(processo.length() - 1, processo.length());
	...
	// Codigo 0 - Formatar padrão SAP, caso contrário, Retira do padrao SAP e joga
	// no Padrão exibido ao
	// usuário
	...
	/**
	 * Retirado da seguinte implementação
	 * https://github.com/popofibo/TrimObjectGraph/blob/master/src/com/popofibo/utils/SpaceUtil.java
	 * 
	 */
	public static Object trimReflective(Object object) {
		...
		try {
			...
			// Introspector usage to pick the getters conveniently thereby excluding the
			// Object getters
	 ...
	 * @return
	 */
	public static boolean isMesAniversarioEdataMenorQueDataAtual(int diaAniversario, int mesAniversario,
			int anoAniversario) {
		LocalDate endDate = LocalDate.now();
		...
		/**
		 * data nascimento 13/05/1982 data atual 20/07/2022 checar se o mês do
		 * aniversario é igual ao mês atual
		 */
	...
	public static String getStringMD5HashFromObject(final byte[] bytesObjeto) {
		MessageDigest md;
		try {
			md = MessageDigest.getInstance("MD5");
			md.update(bytesObjeto);
			final byte[] digest = md.digest();
			final String myHash = DatatypeConverter.printHexBinary(digest).toUpperCase();
