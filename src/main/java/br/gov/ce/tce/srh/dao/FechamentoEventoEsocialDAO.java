...
@Repository
public class FechamentoEventoEsocialDAO {

	static Logger logger = Logger.getLogger(FechamentoEventoEsocialDAO.class);
	...
	public Long getMaxId() {
		Query query = entityManager.createQuery("Select max(e.id) from FechamentoEventoEsocial e ");
	...
	public FechamentoEventoEsocial getById(Long id) {
		return entityManager.find(FechamentoEventoEsocial.class, id);
	}
	...
	public FechamentoEventoEsocial getEventoS1299ByServidor(Funcional servidorFuncional) {
		try {
			Query query = entityManager.createNativeQuery(getSQLEventoS1299(), FechamentoEventoEsocial.class);
			query.setParameter("idFuncional",servidorFuncional.getId() );
			return (FechamentoEventoEsocial) query.getSingleResult();
	...
	public String getSQLEventoS1299() {
		...
		sql.append(" CASE  ");
		sql.append(" WHEN a.datainicioFechamentoEventoEsocial  >= TO_DATE('"+dataLimiteEsocial+"', 'dd/mm/yyyy') THEN 'N'  ");
		sql.append(" ELSE 'S' ");
		sql.append(" END AS CAD_INI, ");
		sql.append(" NULL AS INC_SIT_BENEF, ");
		sql.append(" a.NRFechamentoEventoEsocial AS NR_FechamentoEventoEsocial, ");
		
		sql.append(" a.datainicioFechamentoEventoEsocial AS DT_INI_FechamentoEventoEsocial, ");
		
        /*
         * sql.append(" CASE  "); sql.append(" WHEN a.datainicioFechamentoEventoEsocial  > TO_DATE('"
         * +dataLimiteEsocial+"', 'dd/mm/yyyy') THEN a.datainicioFechamentoEventoEsocial  ");
         * sql.append(" ELSE TO_DATE('"+dataLimiteEsocial+"', 'dd/mm/yyyy') ");         
		 * sql.append(" END AS DT_INI_FechamentoEventoEsocial, ");		*/
		sql.append("  a.datapublicacaoato AS DT_PUBLIC, ");
		sql.append("  a.codigoesocial AS TP_FechamentoEventoEsocial, ");
		...
		sql.append("  NULL AS NR_FechamentoEventoEsocial_ANT_SUC, ");
		sql.append("  NULL AS DT_TRANSF, ");
		sql.append("  NULL AS OBSERVACAO_SUC, ");
		sql.append("  NULL AS CPF_ANT, ");
		sql.append("  NULL AS NR_FechamentoEventoEsocial_ANT_MUD, ");
		sql.append("   NULL AS DT_ALT_CPF, ");
		sql.append("  NULL AS OBSERVACAO_MUD, ");
		sql.append("  NULL AS DT_TERM_FechamentoEventoEsocial, ");
