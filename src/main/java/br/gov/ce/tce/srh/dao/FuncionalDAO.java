...
public class FuncionalDAO {
  ...
  /**
   * Método responsável em pegar a última matrícula cadastrada e verificar qual é a próxima matrícula
   * válida utilizando-se para isso o validador da matrícula
   * 
   * @return
   * 
   */
  ...
  public String getMaxMatricula() {
    ...
    if (matricula != null) {
      ...
      do {
        ...
        // adicionando quantidades de zeros restante para comparar a matrícula no padrão NNNN-D onde N é um
        // número e D é o dígito verificador
  ...
  public int count(Long pessoal, String orderBy) {
    Query query = entityManager.createQuery("SELECT count (f) " + "FROM Funcional f " + "WHERE f.pessoal.id = :idPessoal " + "ORDER BY f.id " + orderBy);
    query.setParameter("idPessoal", pessoal);
    return ((Long) query.getSingleResult()).intValue();
  }

  public List<Funcional> search(Long pessoal, String orderBy, int first, int rows) {
    TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.tipoMovimentoEntrada, f.ocupacao, f.exercicio, f.saida) " + "FROM Funcional f "
                              + "WHERE f.pessoal.id = :idPessoal " + "ORDER BY f.id " + orderBy, Funcional.class);
    query.setParameter("idPessoal", pessoal);
  ...
  public List<Funcional> searchForReclassificacao(Long pessoal, String orderBy, int first, int rows) {
    TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.tipoMovimentoEntrada, f.ocupacao, f.exercicio, f.saida, f.posse) " + "FROM Funcional f "
                              + "WHERE f.pessoal.id = :idPessoal " + "ORDER BY f.id " + orderBy, Funcional.class);
    query.setParameter("idPessoal", pessoal);
    query.setFirstResult(first);
    query.setMaxResults(rows);
  ...
  public Funcional getByPessoaAtivoFp(Long idPessoal) {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT f " + "FROM Funcional f " + "WHERE f.pessoal.id = :idPessoal " + "AND f.saida is null " + "AND f.status != 5 "
                                + "AND f.atipoFp = 0 " + "AND (f.situacao.id = 5 OR f.situacao.id = 7) " + "ORDER BY f.id", Funcional.class);
  ...
  public List<Funcional> findAllAtivos() {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal) " + "FROM Funcional f " + "WHERE f.saida IS NULL " + "AND f.status = 1 "
                                + "AND f.setor.id != 113 " + "ORDER BY f.pessoal.nome", Funcional.class);
      return query.getResultList();
    } catch (NoResultException e) {
      return null;
    }
  ...
  public Funcional getByMatriculaAtivo(String matricula) {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT f " + "FROM Funcional f " + "WHERE upper( f.matricula ) = :matricula " + "AND f.saida IS NULL " + "AND f.status != 5 ",
                                Funcional.class);
  ...
  public Funcional getMatriculaAndNomeByCpf(String cpf) {
    ...
    TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nomeCompleto, f.setor, f.ocupacao, f.exercicio) " + "FROM Funcional f "
                              + "WHERE f.pessoal.cpf = :cpf " + "ORDER BY f.id desc", Funcional.class);
  ...
  public Funcional getMatriculaAndNomeByCpfAtiva(String cpf) {
    ...
    TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nomeCompleto, f.setor, f.ocupacao, f.exercicio) " + "FROM Funcional f "
                              + "WHERE f.pessoal.cpf = :cpf " + "AND f.saida IS NULL " + "AND f.status != 5 " + "ORDER BY f.id desc", Funcional.class);
    query.setParameter("cpf", SRHUtils.removerMascara(cpf));
    List<Funcional> lista = query.getResultList();
    if (lista.size() > 0)
      return lista.get(0);
  ...
  public Funcional getCpfAndNomeByMatricula(String matricula) {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nomeCompleto, f.setor, f.ocupacao, f.exercicio) " + "FROM Funcional f "
                                + "WHERE upper( f.matricula ) = :matricula " + "ORDER BY f.id desc", Funcional.class);
      query.setParameter("matricula", matricula);
      List<Funcional> lista = query.getResultList();
      if (lista.size() > 0)
        return lista.get(0);
  ...
  public Funcional getCpfAndNomeByMatriculaAtiva(String matricula) {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nomeCompleto, f.setor, f.ocupacao, f.exercicio) " + "FROM Funcional f "
                                + "WHERE upper( f.matricula ) = :matricula " + "AND f.saida IS NULL " + "AND f.status != 5 " + "ORDER BY f.id desc", Funcional.class);
      query.setParameter("matricula", matricula);
      return query.getSingleResult();
    } catch (NoResultException e) {
      return null;
  ...
  public List<Funcional> findByNome(String nome) {
    TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.setor, f.ocupacao, f.vinculo) " + "FROM Funcional f "
                              + "WHERE upper( f.nomePesquisa ) like :nome " + "AND f.saida IS NULL " + "AND f.status != 5 " + "ORDER BY f.id desc", Funcional.class);
  ...
  public List<Funcional> findAllByNome(String nome) {
    TypedQuery<Funcional> query = entityManager.createQuery(
                              "SELECT new Funcional(f.id, f.matricula, f.pessoal, f.setor) " + "FROM Funcional f " + "WHERE upper( f.nomePesquisa ) like :nome " + "ORDER BY f.id desc",
                              Funcional.class);
    query.setParameter("nome", "%" + nome.toUpperCase() + "%");
  ...
  /***
   * Ocupação 14 - Estagiário Nível Médio Ocupação 15 - Estagiário Nível Universitário
   * 
   * @return
   */
  public List<Funcional> findServidoresEvento2200() {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nome) " + "FROM Funcional f " + "WHERE f.saida IS NULL " + "AND f.status = 1 "
                                + "AND f.ocupacao.id not in (14,15) " + "AND f.id  NOT IN (SELECT a.funcional.id FROM Admissao a) " + "ORDER BY f.nome", Funcional.class);
      return query.getResultList();
    } catch (NoResultException e) {
      return null;
    }
  ...
  public List<Funcional> findServidoresEvento2399() {
    TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nome) " + "FROM Funcional f " + "WHERE "
    // + " f.status < 3 " //pegar todos que sao 1,2
                              + "  f.id IN (SELECT a.funcional.id FROM EstagiarioESocial a) " + " AND f.saida > to_date('21/11/2021', 'dd/mm/yyyy') "
                              + " AND f.id  NOT IN (SELECT a.funcional.id FROM TerminoVinculo a) " + "ORDER BY f.nome", Funcional.class);
    return query.getResultList();
  }
  ...
  public List<Funcional> findServidoresEvento2230() {
    try {
      ...
      sql.append("FROM tb_licenca l ");
      sql.append("    INNER JOIN tb_tipolicenca tl on l.idtipolicenca = tl.id ");
      ...
      sql.append("    AND f.id NOT IN (SELECT idFuncional FROM esocial_afastamento where TO_CHAR(dt_ini_afast,'DD/MM/YYYY') = TO_CHAR(l.inicio, 'DD/MM/YYYY') )");      
      sql.append("ORDER BY ");
      sql.append("  f.nome ASC ");
  ...
  public List<Funcional> findServidoresEventoAuxilioSaude() {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nome) " + "FROM Funcional f " + "WHERE f.saida IS NULL " + "AND f.status = 1 "
                                + " OR f.ocupacao.id in (14,15)" + "ORDER BY f.nome", Funcional.class);
      return query.getResultList();
    } catch (NoResultException e) {
      return null;
    }
  ...
  public List<Funcional> findBeneficiariosEvento2400() {
    /*
     * try { TypedQuery<Funcional> query =
     * entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nome) " +
     * "FROM Funcional f " + "WHERE f.status = 5 and f.saida is null " +
     ...
     * "AND f.id  NOT IN (SELECT b.funcional.id FROM Beneficiario b) " + "ORDER BY f.nome",
     * Funcional.class); return query.getResultList(); } catch (NoResultException e) { return null; }
     */

    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nome) " + "FROM Funcional f " + "WHERE f.status = 5 "
                                + "AND f.pessoal.id  IN (SELECT a.funcional.pessoal.id FROM Aposentadoria a) " + "AND f.id  NOT IN (SELECT b.funcional.id FROM Beneficiario b) "
                                + "AND  f.saida is null " + "ORDER BY f.nome", Funcional.class);
      return query.getResultList();
    } catch (NoResultException e) {
      return null;
    }
  ...
  public List<Funcional> findEstagiariosEventos2300() {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nome) " + "FROM Funcional f " + "WHERE f.saida IS NULL " + "AND f.status = 2 "
                                + "AND f.ocupacao.id in (14,15) " + "AND f.id  NOT IN (SELECT e.funcional.id FROM EstagiarioESocial e) " + "ORDER BY f.nome", Funcional.class);
      return query.getResultList();
    } catch (NoResultException e) {
      return null;
    }
  ...
  public List<Funcional> findBeneficiariosEvento2410() {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nome) " + "FROM Funcional f " + "WHERE f.status = 5"
                                + "AND f.id  IN (SELECT b.funcional.id FROM Beneficiario b) " + "AND f.id  NOT IN (SELECT b.funcional.id FROM Beneficio b) " + "ORDER BY f.nome", Funcional.class);
      return query.getResultList();
    } catch (NoResultException e) {
      return null;
    }
  ...
  public List<Funcional> findServidorEvento2300() {
    try {
      TypedQuery<Funcional> query = entityManager.createQuery("SELECT new Funcional(f.id, f.matricula, f.pessoal, f.nome) " + "FROM Funcional f " + "WHERE f.saida IS NULL " + "AND f.status = 1 "
                                + "AND f.id IN (SELECT fc.funcional.id FROM FuncionalCedido fc)" + "AND f.id  NOT IN (SELECT e.funcional.id FROM EstagiarioESocial e) " + "ORDER BY f.nome",
                                Funcional.class);
      return query.getResultList();
    } catch (NoResultException e) {
      return null;
