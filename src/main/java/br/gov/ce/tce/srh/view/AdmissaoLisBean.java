...
@Component("admissaoListBean")
@Scope("view")
public class AdmissaoLisBean implements Serializable {
	...
	public void excluir() {
		...
		try {
			...
			admissaoESocialTCEService.excluir(entidade);
	...
	public List<Admissao> getLista(){return lista;}
	
	//PAGINAÇÃO
	private void limparListas() {
		dataTable = new UIDataTable();
		dataModel = new PagedListDataModel();
		pagedList = new ArrayList<Admissao>(); 
	}
	...
	public PagedListDataModel getDataModel() {
		if( flagRegistroInicial != getDataTable().getFirst() ) {
			...
			setPagedList(admissaoESocialTCEService.search(this.nome, this.cpf, getDataTable().getFirst(), getDataTable().getRows()));
			if(count != 0){
