...
public class PessoalService {
  ...
  private void validarDados(Pessoal entidade) {
    ...
    if (!entidade.getCategoria().getId().equals(4L) && !entidade.getCategoria().getId().equals(5L)) {

      if (entidade.getEscolaridade() == null)
        throw new SRHRuntimeException("A Escolaridade é obrigatória.");

      if (entidade.isPermitidoValidarCampoTipoDeficiencia() && entidade.getTipoDeficiencia() == null)
        throw new SRHRuntimeException("O Tipo Deficiência é obrigatório.");


      /*
       * if(entidade.getIngressouPorCotas() == null || entidade.getIngressouPorCotas().isEmpty()) throw
       * new SRHRuntimeException("É necessário informar se você entrou por cotas.");
       */

      /*
       * if(entidade.getIsReadaptado() == null || entidade.getIsReadaptado().isEmpty()) throw new
       * SRHRuntimeException("É necessário informar se você é readaptado.");
       */
      ...
      if (entidade.getTelefone() != null && !entidade.getTelefone().isEmpty() && entidade.getTelefoneAlternativo() != null && !entidade.getTelefoneAlternativo().isEmpty()
      ...
      if (entidade.getEmail() != null && !entidade.getEmail().isEmpty() && entidade.getEmailAlternativo() != null && !entidade.getEmailAlternativo().isEmpty()
  ...
  private void validarCTPS(Pessoal entidade) {
    if (((entidade.getNrCTPS() != null && !entidade.getNrCTPS().isEmpty()) || (entidade.getSerieCTPS() != null && !entidade.getSerieCTPS().isEmpty()) || entidade.getUfCTPS() != null)
                              && (entidade.getNrCTPS() == null || entidade.getNrCTPS().isEmpty() || entidade.getSerieCTPS() == null || entidade.getSerieCTPS().isEmpty()
  ...
  private void validarRG(Pessoal entidade) {
    if (((entidade.getRg() != null && !entidade.getRg().isEmpty()) || entidade.getDataEmissaoRg() != null || (entidade.getEmissorRg() != null && !entidade.getEmissorRg().isEmpty())
                              || entidade.getUfEmissorRg() != null)
                              && (entidade.getRg() == null || entidade.getRg().isEmpty() || entidade.getEmissorRg() == null || entidade.getEmissorRg().isEmpty() || entidade.getUfEmissorRg() == null))
  ...
  private void validarRegistroEmOrdemDeClasse(Pessoal entidade) {
    ...
    if ((entidade.getDataExpedicaoOrgaoClasse() != null || entidade.getDataValidadeOrgaoClasse() != null || (entidade.getEmissorOrgaoClasse() != null && !entidade.getEmissorOrgaoClasse().isEmpty())
                              || (entidade.getNrOrgaoClasse() != null && !entidade.getNrOrgaoClasse().isEmpty()) || entidade.getUfOrgaoClasse() != null)
                              && (entidade.getNrOrgaoClasse() == null || entidade.getNrOrgaoClasse().isEmpty() || entidade.getEmissorOrgaoClasse() == null || entidade.getEmissorOrgaoClasse().isEmpty()
                                                        || entidade.getUfOrgaoClasse() == null))
      throw new SRHRuntimeException("Caso sejam prenchidas Informações do número de registro em Órgão de Classe, " + "os campos Número Inscrição, Órgão Emissor e UF passam a ser obrigatórios.");
  ...
  private void validarCNH(Pessoal entidade) {
    ...
    if ((entidade.getCategoriaCNH() != null || entidade.getDataExpedicaoCNH() != null || entidade.getDataPrimeiraCNH() != null || entidade.getDataValidadeCNH() != null
                              || (entidade.getNrCNH() != null && !entidade.getNrCNH().isEmpty()) || entidade.getUfCNH() != null)
                              && (entidade.getCategoriaCNH() == null || entidade.getDataValidadeCNH() == null || entidade.getNrCNH() == null || entidade.getNrCNH().isEmpty()
                                                        || entidade.getUfCNH() == null))
      throw new SRHRuntimeException("Caso sejam prenchidas Informações da Carteira Nacional de Habilitação, " + "os campos Número Registro, UF, Data Validade e Categoria passam a ser obrigatórios.");
  ...
  public void setDAO(PessoalDAO pessoalDAO) {
    this.dao = pessoalDAO;
  }
