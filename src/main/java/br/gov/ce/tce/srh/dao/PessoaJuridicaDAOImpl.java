...
public class PessoaJuridicaDAOImpl implements PessoaJuridicaDAO {
  ...
  public void excluir(PessoaJuridica entidade) {
    entityManager.createQuery("DELETE FROM PessoaJuridica pj WHERE pj.id=:id ").setParameter("id", entidade.getId()).executeUpdate();
  }
  ...
  public PessoaJuridica getByrazaoSocial(String razaoSocial) {
    try {
      Query query = entityManager.createQuery("Select pj from PessoaJuridica pj where pj.razaoSocial = :razaoSocial ");
      query.setParameter("razaoSocial", razaoSocial.toUpperCase());
      return (PessoaJuridica) query.getSingleResult();
    } catch (NoResultException e) {
      return null;
  ...
  public PessoaJuridica getBynomeFantasia(String nomeFantasia) {
    try {
      Query query = entityManager.createQuery("Select pj from PessoaJuridica pj where pj.nomeFantasia = :nomeFantasia ");
      query.setParameter("nomeFantasia", nomeFantasia.toUpperCase());
      return (PessoaJuridica) query.getSingleResult();
    } catch (NoResultException e) {
      return null;
  ...
  public List<PessoaJuridica> findAllNotTipo(List<TipodeEmpresa> tipodeEmpresaList) {
    Query query = entityManager.createQuery("SELECT pj FROM PessoaJuridica pj  where pj.tipodeEmpresa not in(:tipodeEmpresaList) ORDER BY pj.id");
    query.setParameter("tipodeEmpresaList", tipodeEmpresaList);
    return query.getResultList();
  }
  ...
  public List<PessoaJuridica> findAllByTipo(TipodeEmpresa tipodeEmpresa) {
    Query query = entityManager.createQuery("SELECT pj FROM PessoaJuridica pj where pj.tipodeEmpresa = :tipodeEmpresa ORDER BY pj.id", PessoaJuridica.class);
  ...
  public PessoaJuridica findById(Long id) {
    try {
      Query query = entityManager.createQuery("Select pj from PessoaJuridica pj where pj.id = :id ");
