...
@Repository
public class EstatutarioESocialDAO {
	static Logger logger = Logger.getLogger(EstatutarioESocialDAO.class);
	...
	public EstatutarioESocial getEventoS2300ByEstatutario(Funcional estatutarioFuncional) {
		try {
			Query query = entityManager.createNativeQuery(getSQLEventoS2300(), EstatutarioESocial.class);
			query.setParameter("idFuncional", estatutarioFuncional.getId() );
			return (EstatutarioESocial) query.getSingleResult();
	...
	private Long getMaxId() {
		Query query = entityManager.createQuery("Select max(e.id) from EstatutarioESocial e ");
