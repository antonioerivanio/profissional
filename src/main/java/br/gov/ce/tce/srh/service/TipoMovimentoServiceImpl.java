package br.gov.ce.tce.srh.service;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import br.gov.ce.tce.srh.dao.TipoMovimentoDAO;
import br.gov.ce.tce.srh.domain.TipoMovimento;
import br.gov.ce.tce.srh.exception.SRHRuntimeException;

/**
 * 
 * @author robstown
 *
 */
@Service("tipoMovimentoService")
public class TipoMovimentoServiceImpl implements TipoMovimentoService {

	@Autowired
	private TipoMovimentoDAO dao;


	@Override
	@Transactional
	public void salvar(TipoMovimento entidade) throws SRHRuntimeException {

		/*
		 * Regra: 
		 *
		 * Nao deixar cadastrar entidade ja existente.
		 * 
		 */
		verificandoSeEntidadeExiste(entidade);
		
		// persistindo
		dao.salvar(entidade);
	}


	@Override
	@Transactional
	public void excluir(TipoMovimento entidade) {
		dao.excluir(entidade);
	}


	@Override
	public int count(String descricao) {
		return dao.count(descricao);
	}


	@Override
	public List<TipoMovimento> search(String descricao, int first, int rows) {
		return dao.search(descricao, first, rows);
	}


	@Override
	public TipoMovimento getById(Long id) {
		return dao.getById(id);
	}


	@Override
	public List<TipoMovimento> findByTipo(Long tipo) {
		return dao.findByTipo(tipo);
	}
	
	public List<TipoMovimento> findByDescricaoForId(Long id) {
		return dao.findByDescricaoForId(id);
	}

	@Override
	public List<TipoMovimento> findAll() {
		return dao.findAll();
	}



	/**
	 * Regra de Negocio: 
	 * 
	 * Verifica na base se nao existe tipo movimento cadastrado com a mesma DESCRICAO
	 * 
	 * @throws SRHRuntimeException
	 * 
	 */
	private void verificandoSeEntidadeExiste(TipoMovimento entidade) throws SRHRuntimeException {

		TipoMovimento entidadeJaExiste = dao.getByDescricao(entidade.getDescricao());
		if (entidadeJaExiste != null && !entidade.getId().equals(entidadeJaExiste.getId()))
			throw new SRHRuntimeException("Tipo de Movimento já cadastrado. Operação cancelada.");
		
	}


	public void setDAO(TipoMovimentoDAO tipoMovimentoDAO) {this.dao = tipoMovimentoDAO;}


	

}
