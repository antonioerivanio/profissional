...
public class TipoDocumentoTest extends DatabaseTestCase {
	...
	public void testChavePrimaria() throws SQLException, Exception {
		...
		ITable tableAlunoAtual = baseAtual.getTable("TB_TIPODOCUMENTO");
	...
	public void testTipoColuna() throws SQLException, Exception {
		...
		String esfera = colunas[2].getSqlTypeName();
		String docFuncional = colunas[3].getSqlTypeName();

		Assert.assertEquals("NUMBER", id);
		Assert.assertEquals("VARCHAR2", descricao);
		Assert.assertEquals("NUMBER", esfera);
		Assert.assertEquals("NUMBER", docFuncional);
