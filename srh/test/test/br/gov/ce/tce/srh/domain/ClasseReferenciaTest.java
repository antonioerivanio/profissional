...
public class ClasseReferenciaTest extends DatabaseTestCase {
	...
	public void testChavePrimaria() throws SQLException, Exception {
		...
		ITable tableAlunoAtual = baseAtual.getTable("TB_CLASSEREFERENCIA");
	...
	public void testTipoColuna() throws SQLException, Exception {
		...
		String referencia = colunas[2].getSqlTypeName();
		String classe = colunas[3].getSqlTypeName();
		String simbolo = colunas[4].getSqlTypeName();
		String horas = colunas[5].getSqlTypeName();

		Assert.assertEquals("NUMBER", id);
		Assert.assertEquals("NUMBER", escolaridade);
		Assert.assertEquals("NUMBER", referencia);
		Assert.assertEquals("CHAR", classe);
		Assert.assertEquals("NUMBER", simbolo);
		Assert.assertEquals("NUMBER", horas);
