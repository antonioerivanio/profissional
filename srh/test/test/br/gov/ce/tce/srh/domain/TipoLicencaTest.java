...
public class TipoLicencaTest extends DatabaseTestCase {
	...
	public void testChavePrimaria() throws SQLException, Exception {
		...
		ITable tableAlunoAtual = baseAtual.getTable("TB_TIPOLICENCA");
	...
	public void testTipoColuna() throws SQLException, Exception {
		...
		String fundamentacao = colunas[2].getSqlTypeName();
		String sexoValido = colunas[3].getSqlTypeName();
		String qtdMaximoDias = colunas[4].getSqlTypeName();
		String especial = colunas[5].getSqlTypeName();

		Assert.assertEquals("NUMBER", id);
		Assert.assertEquals("VARCHAR2", descricao);
		Assert.assertEquals("VARCHAR2", fundamentacao);
		Assert.assertEquals("CHAR", sexoValido);
		Assert.assertEquals("NUMBER", qtdMaximoDias);
		Assert.assertEquals("NUMBER", especial);
