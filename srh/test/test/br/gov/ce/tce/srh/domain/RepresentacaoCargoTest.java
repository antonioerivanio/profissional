...
public class RepresentacaoCargoTest extends DatabaseTestCase {
	...
	public void testChavePrimaria() throws SQLException, Exception {
		...
		ITable tableAlunoAtual = baseAtual.getTable("TB_REPRESENTACAOCARGO");
	...
	public void testTipoColuna() throws SQLException, Exception {
		...
		String ordem = colunas[1].getSqlTypeName();
		String simbolo = colunas[2].getSqlTypeName();
		String ativo = colunas[3].getSqlTypeName();
		String moneclatura = colunas[4].getSqlTypeName();
		String observacao = colunas[5].getSqlTypeName();

		Assert.assertEquals("NUMBER", id);
		Assert.assertEquals("NUMBER", ordem);
		Assert.assertEquals("VARCHAR2", simbolo);
		Assert.assertEquals("NUMBER", ativo);
		Assert.assertEquals("VARCHAR2", moneclatura);
